...
public class BoltCypherExecutor implements CypherExecutor {
    ...
    public BoltCypherExecutor(String url, String username, String password) {
        ...
        driver = GraphDatabase.driver(url, token, Config.build().withEncryptionLevel(Config.EncryptionLevel.NONE)
                .toConfig());
    }
    ...
    @Override
    public StatementResult queryResult(String query, Map<String, Object> params) {
        try (Session session = driver.session()) {
